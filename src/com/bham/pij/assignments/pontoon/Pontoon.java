...
public class Pontoon extends CardGame{
	...
	public int compareHands(Player hand1, Player hand2) {
		...
		//if statement for the ranking of hands
		if (checkForBust(numValue) == false && checkForBust(numValueTwo) == true)
			return -1;
		else if (checkForBust(numValue) == true && checkForBust(numValueTwo) == true)
			return 0;
		else if (checkForBust(numValueTwo) == false && checkForBust(numValue) == true)
			return 1;
		else if(((un == Value.ACE && deux == Value.TEN) || (un == Value.TEN && deux == Value.ACE)) && ((trois != Value.ACE && quatre != Value.ACE)))
			return -1;
		else if (((un == Value.ACE && deux == Value.TEN) || (un == Value.TEN && deux == Value.ACE)) && ((trois == Value.ACE && quatre == Value.TEN) || (trois == Value.TEN && quatre == Value.ACE)))
			return 0;
		else if (((un != Value.ACE &&  deux != Value.ACE)) && ((trois == Value.ACE && quatre == Value.TEN) || (trois == Value.TEN && quatre == Value.ACE)))
			return 1;
		else if (handOne.size() == 5 && twentyOneCheck(numValue) == true && handTwo.size() !=5)
			return -1;
		else if (handOne.size() == 5 && twentyOneCheck(numValue) == true && handTwo.size() ==5 && twentyOneCheck(numValueTwo) == true)
			return 0;
		else if (handTwo.size() == 5 && twentyOneCheck(numValueTwo) == true && handOne.size() !=5)
			return 1;
		else if (twentyOneCheck(numValue) == true && twentyOneCheck(numValueTwo) == false)
			return -1;
		else if (twentyOneCheck(numValue) == true && twentyOneCheck(numValueTwo) == true)
			return 0;
		else if (twentyOneCheck(numValueTwo) == true && twentyOneCheck(numValue) == false)
			return 1;
		else if(handOneCheck > handTwoCheck)
			return -1;
		else if (handOneCheck == handTwoCheck)
			return 0;
		else if(handTwoCheck > handOneCheck)
			return 1;
		return 0;
